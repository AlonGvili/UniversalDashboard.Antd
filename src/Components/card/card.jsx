...
const renderCardContent = props => {
	const { parameterSet, metaAvatar, metaDescription, metaTitle, gridCards } = props.attributes
	...
	switch (parameterSet) {
		case "Default":
			cardContent = Array.isArray(props.content)
				? props.content.map(item => (item.type ? UniversalDashboard.renderComponent(item) : item))
				: props.content.type
		...
		case "Meta":
			cardContent = (
				<Card.Meta
					avatar={metaAvatar && UniversalDashboard.renderComponent(metaAvatar)}
					title={(metaTitle && metaTitle.type && UniversalDashboard.renderComponent(metaTitle)) || metaTitle}
					description={
						(metaDescription &&
							Array.isArray(metaDescription) &&
							UniversalDashboard.renderComponent(metaDescription)) ||
						(metaDescription.type && UniversalDashboard.renderComponent(metaDescription)) ||
						metaDescription
					}
				/>
			)
				...
				>
					{Array.isArray(item.content)
						? item.content.map(gridItem =>
								gridItem.type ? UniversalDashboard.renderComponent(gridItem) : gridItem
						  )
						: item.content.type
...
const AntdCard = props => {
	...
	const tabsProps = {
		...
		tabBarExtraContent: tabBarExtraContent ? UniversalDashboard.renderComponent(tabBarExtraContent) : null,
	}

	return (
		<Fragment>
			<Card {...otherProps} {...cardProps} {...tabsProps}>
				{parameterSet === "Tabs" ? renderTabContent(activeTab.key, tabs) : renderCardContent(state)}
			</Card>

...
const AntdMenu = props => {
	...
	const renderMenuLink = item => (
		<Menu.Item style={ { ...item.style } }>
			<Link to={ `${window.baseUrl}${item.to}` } style={ { color: "inherit", width: "100%", height: "auto" } }>
	...
	const renderMenuContent = item => (
		<Menu.Item key={ item.key } style={ { ...item.style } } icon={ item.icon && UniversalDashboard.renderComponent(item.icon) }>
			{ item.text && UniversalDashboard.renderComponent(item.text) }
